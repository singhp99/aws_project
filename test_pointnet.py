...
def train_best_model():
    ...
    train_features_get = read_s3("16O_size800_train_features.npy")
    train_features = np.load(BytesIO(train_features_get))
    
    train_labels_get = read_s3("16O_size800_train_labels.npy")
    train_labels = np.load(BytesIO(train_labels_get))
    ...
    val_features_get = read_s3("16O_size800_val_features.npy")
    val_features = np.load(BytesIO(val_features_get))
    
    val_labels_get = read_s3("16O_size800_val_labels.npy")
    val_labels = np.load(BytesIO(val_labels_get))
    ...
    write_s3("best_model.keras", "best_model.keras") #writing to s3 bucket
    
    plot_learning_curve(history, 'learning-curve.png', batch_size_num,learning_rate_num)
    
    write_s3("learning-curve.png","learning-curve.png") #writing learning curve to s3 bucket
